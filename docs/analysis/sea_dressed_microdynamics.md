...
The loophole is standard chemical kinetics: a bilinear rate `k · B · N`
...
- **polarized sea excitations** — coherent pair excitations carrying momentum
  `±q h/L`, whose local density realizes `Gamma_q(x)` as a *prepared state of
...
| (polarized excitation) | excited bound pair carrying `±q h/L` | `0` |

The observable excess field is `E = U+ − U−`, and the Wigner distribution is
`W = E / (nu · dx · dp)`. Wigner negativity is a local negaton surplus —
...
The sea is initialized virgin: `S = B := round(nu · (2/h) · dx · dp)` in every
...
- **Level 1 (pinned sea).** `S/B ≡ 1` in all rates, and recombination is
  instantaneous (`κ → ∞`). This is the regime of the exactness theorem (§5).
...
| ch. | rate | effect on `U+ / U− / S` | `ΔE` stencil | collision reading |
|---|---|---|---|---|
| **K1** | `(γ/2) s_lo U⁺_hi` | `U⁺_hi−1, U⁺_n+2, U⁻_lo+1, S_lo−1` | focus `(−1,+2,−1)` | unpaired positon at *hi* and sea positon at *lo* scatter into `n`; the sea partner's negaton is orphaned at *lo* |
| **K1b** | `(γ/2) s_lo U⁻_hi` | `U⁻_hi−1, U⁻_n+2, U⁺_lo+1, S_lo−1` | defocus `(+1,−2,+1)` | crossing conjugate of K1 (negaton capture) |
| **K2** | `(γ/2) s_n² U⁺_lo` | `U⁺_hi+1, U⁺_lo+1, U⁻_n+2, S_n−2` | defocus | two sea positons at `n` scatter to *hi*/*lo*, Bose-stimulated by the unpaired-positon occupancy at *lo*; both negatons orphaned at `n` |
| **K2b** | `(γ/2) s_n² U⁻_lo` | `U⁻_hi+1, U⁻_lo+1, U⁺_n+2, S_n−2` | focus | crossing conjugate of K2 |
| **K3** | `(γ/2) U⁺_hi` | `U⁺_hi−1, U⁺_lo+1` | hop *hi*→*lo* `(−1,0,+1)` | unpaired positon absorbs a polarized excitation carrying `−σ·2q\,dp` |
| **K3b** | `(γ/2) U⁻_hi` | `U⁻_hi−1, U⁻_lo+1` | hop *lo*→*hi* on `E` | crossing conjugate of K3 |
| **K4** | `(γ/2) s_hi U⁺_lo` | `U⁺_lo+1, U⁻_hi+1, S_hi−1` | hop *hi*→*lo* | a sea positon at *hi* emits an excitation and lands at *lo*, destination-stimulated by `U⁺_lo`; its negaton is orphaned at *hi* |
| **K4b** | `(γ/2) s_hi U⁻_lo` | `U⁻_lo+1, U⁺_hi+1, S_hi−1` | hop *lo*→*hi* on `E` | crossing conjugate of K4 |

**Momentum bookkeeping (every row is a genuine two-body event):**

- K1/K1b: the two captured particles change momentum by `−σq\,dp` and
  `+σq\,dp`; total `0`.
- K2/K2b: the two sea partners recoil to `±σq\,dp`; total `0`.
- K3/K3b: the particle's `−σ·2q\,dp` is supplied by the absorbed excitation.
- K4/K4b: the particle's `−σ·2q\,dp` is carried off by the emitted excitation.

**Recombination channel** (the hidden sea microdynamics, level 2):

| ch. | rate (per cell) | effect | `ΔE` |
|---|---|---|---|
| **R** | `κ · U⁺_n U⁻_n / B` | `U⁺_n−1, U⁻_n−1, S_n+1` | `0` |
...
At level 1 (`s ≡ 1`), sum the signed `ΔE` contributions of the block. Per
...
`F = (A - A^{-1})G`, whose Fourier symbol vanishes at `q\theta \in \{0,
\pi\}`; imposing that on the symbol of `F` forces `a_0 = 0` and
...
capture: rate `∝ (density of unpaired positons at hi) × (density of sea
...
absorption from the polarized-excitation reservoir: rate `∝ (reservoir
density) × (absorber density)`, with the reservoir density `∝ γ` pinned.
...
destination cell: physical rate `∝ B(1 + U^+_{lo}/\mathcal{N})` with
...
stimulate K2 by *both* destinations, `\propto (1 + U_{lo}/\mathcal{N})(1 +
U_{hi}/\mathcal{N})`, whose expansion contains a bilinear term
...
variant* of the table reweights K1 to rate `γ s_{lo} U^+_{hi}` and K2 to
`(γ/2)(U^+_{lo} + U^+_{hi})`; its mean field is identical), and the bilinear
...
1. **Depletion.** Rates carry `s = S/B`; as `S` drains where the dynamics is
   ...
   fastest through `s²`, K3 not at all — so depletion also *distorts*, not
   merely slows.)
...
(`ΔE = 0` per event). Level 1 is recovered as the `κ → ∞` limit — pinning is
...
`κ = 0 \to 20 \to 200 \to` pinned quantitatively.
...
2. **Jump substep** (`step_jump_sea_mc`). Per mode and polarization block,
   ...
   K3, K3b, K4, K4b: draw `e ~ Poisson(rate · dt)` lattice-wide, cap by the
   ...
   `r = min(U^+, U^-)` per cell.
3. **Recombination substep** (`step_recombine`). `e ~ Poisson(κ U^+ U^- dt /
   B)` per cell, capped by `min(U^+, U^-)`.
...
pinned, and live sea at `κ ∈ {0, 20, 200}`. Relative L2 deviation from the
mesh at `t = 2T`:
...
| sea MC, live, `κ = 200` | 0.172 | 0.945 | 5.1 M |
| sea MC, live, `κ = 20`  | 0.352 | 0.796 | 29 M |
| sea MC, live, `κ = 0`   | 0.764 | 0.000 | 206 M |
...
- **`κ = 0` exhibits both failure modes at once**: the worst cell drains the
...
- **Convergence in `κ`.** `0.764 \to 0.352 \to 0.172` approaching the pinned
  0.108, with the sea held at 95% everywhere by `κ = 200 \approx
  130\,|\Gamma|_{max}`. Pinning is visibly a derived, quantitative property
